use candle_core::{CudaDevice, CudaStorage, DType, Device, Error, Result, Tensor, Var, Storage};
...
impl LoRABackwardOps {
    ...
    pub fn backward(
        ...
        Err(Error::Msg("CUDA backward not enabled".into()))
    }
}
...
impl NormBackwardOps {
    ...
    pub fn group_norm_backward(
        ...
        let get_ptr = |t: &Tensor| -> Result<*const std::ffi::c_void> {
            match t.storage() {
                Storage::Cuda(s) => {
                    let (ptr, _) = s.as_cuda_slice::<f32>()?.device_ptr(&stream);
                    Ok(ptr as *const _)
                }
        ...
        let get_mut_ptr = |t: &Tensor| -> Result<*mut std::ffi::c_void> {
            match t.storage() {
                Storage::Cuda(s) => {
                    let (ptr, _) = s.as_cuda_slice::<f32>()?.device_ptr_mut(&stream);
                    Ok(ptr as *mut _)
                }
                _ => Err(Error::Msg("Expected CUDA tensor".into())),
            }
