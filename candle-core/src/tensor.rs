...
impl Tensor {
    ...
    pub(crate) fn make_var(&self) -> Result<Tensor> {
        // If already a variable, just return a clone
        if self.is_variable() {
            return Ok(self.clone());
        }
        ...
        match self.device() {
            Device::Cuda(_) => {
                // Use our custom CUDA operation that handles the complexity
                #[cfg(feature = "cuda")]
                {
                    crate::cuda_backend::cuda_var_from_tensor(self)
                }
            ...
            _ => {
                // CPU and other devices use original implementation
                let shape = self.shape();
                let mut storage = unsafe { self.device().alloc_uninit(shape, self.dtype())? };
                self.storage()
                    .copy_strided_src(&mut storage, 0, self.layout())?;
